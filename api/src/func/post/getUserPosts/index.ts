...
async function getPostIdsByUser(userId: string) {
  ...
  const postIds = relationKeysRes.keys.map(key => getPostIdFromUsersPostsKey(key.name)).filter(key => key) as string[]
...
export async function getUserPosts(request: Request): Promise<Response> {
  ...
  const input = (await transformAndValidate(GetUserPostsInput, json)) as GetUserPostsInput

  const postIds = await getPostIdsByUser(input.userId)

  const postVals = await Promise.all(
    map<string, Promise<string | null>>(postId => getCachedVal(getPostKey(postId), POSTS, 'POSTS'))(postIds),
  )
